...
export function WorkflowStatus() {
  ...
  // Deduplicate workflows by workflow_id + run_id combination
  const deduplicatedWorkflows = workflows.reduce((acc, workflow) => {
    const key = `${workflow.id}-${workflow.runId || 'no-run-id'}`
    if (!acc[key]) {
      acc[key] = workflow
    }
    return acc
  ...
  const uniqueWorkflows = Object.values(deduplicatedWorkflows)

  // Group workflows by status
  const groupedWorkflows = uniqueWorkflows.reduce((acc, workflow) => {
    ...
    return acc
  }, {} as Record<string, typeof uniqueWorkflows>)
  ...
  const statusCounts = {
    ...
    other: uniqueWorkflows.length - (groupedWorkflows.running?.length || 0) - (groupedWorkflows.completed?.length || 0) - (groupedWorkflows.failed?.length || 0)
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Badge variant="outline">{uniqueWorkflows.length} total</Badge>
        </div>
      </div>
            ...
            <div key={status} className="space-y-2">
              ...
              <div className="space-y-2">
                {statusWorkflows.map((workflow) => (
                  <Card key={`${workflow.id}-${workflow.runId || 'no-run-id'}`}>
