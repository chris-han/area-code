...
and application factory for bia-specific endpoints and services.

This module serves as the complete bia application factory, including:
- Dependency management (ClickHouse, Temporal, Redis, Plugins)
- FastAPI application creation with middleware
- bia router registration
...
class ABIApplication:
    ...
    for bia services including ClickHouse, Temporal, Redis, and plugin systems.
    """
    ...
    async def initialize_dependencies(self):
        ...
        try:
            ...
            logger.error(f"Failed to initialize bia dependencies: {e}")
            raise
    ...
    async def cleanup_dependencies(self):
        """
        Cleanup bia system dependencies.
    ...
    def _load_moose_config(self) -> Dict[str, Any]:
        ...
        # Navigate to the data-warehouse service directory from bia_admin
        repo_root = Path(__file__).resolve().parents[2]  # Goes from app.py -> bia_backend -> bia_admin to repo root
...
# Global bia application instance
...
def create_abi_fastapi_app() -> FastAPI:
    """
    Create and configure the complete bia FastAPI application.

    This creates the application with:
    - bia APIs (billing, workflows, plugins, health checks)
    ...
    Returns:
        FastAPI: Configured FastAPI application with all bia endpoints
    """
    ...
    # Create the base FastAPI app with bia configuration
    ...
    logger.info("bia FastAPI application created successfully")
    return app


# Create the complete FastAPI app instance with all bia routers
