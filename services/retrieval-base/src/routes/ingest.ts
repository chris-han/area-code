...
// Validation schemas for incoming data based on existing models
const fooSchema = z.object({
  ...
  score: z.number(),
  largeText: z.string(),
  createdAt: z.union([z.string(), z.date()]).transform(date => 
    typeof date === 'string' ? new Date(date) : date
  ),
  updatedAt: z.union([z.string(), z.date()]).transform(date => 
    typeof date === 'string' ? new Date(date) : date
  ),
});
...
// Request schemas that include action for CRUD operations
const fooIngestSchema = z.object({
  action: z.enum(["index", "delete"]),
  data: fooSchema.optional(),
});

const barIngestSchema = z.object({
  action: z.enum(["index", "delete"]),
  data: barSchema.optional(),
});

// Type inference from schemas
type FooIngestRequest = z.infer<typeof fooIngestSchema>;
type BarIngestRequest = z.infer<typeof barIngestSchema>;

export const ingestRoutes: FastifyPluginAsync = async (fastify) => {
  // Ingest foo data
  fastify.post("/ingest/foo", {
    schema: {
      body: {
        type: "object",
        required: ["action"],
    ...
    try {
      ...
      fastify.log.info(`📥 Received foo ${action} request for ID: ${data?.id}`);
        ...
        case "delete":
          ...
          try {
            ...
            if (error.meta?.body?.result === "not_found") {
              fastify.log.warn(`⚠️  Foo document not found for deletion: ${data!.id}`);
  ...
  fastify.post("/ingest/bar", {
    ...
    try {
      ...
      fastify.log.info(`📥 Received bar ${action} request for ID: ${data?.id}`);
      ...
      switch (action) {
        ...
        case "delete":
          ...
          try {
            ...
            if (error.meta?.body?.result === "not_found") {
              fastify.log.warn(`⚠️  Bar document not found for deletion: ${data!.id}`);
              result = { result: "not_found" };
            } else {
              throw error;
            }
