...
- **Moose runtime** (`app/main.py`): registers ingestion pipelines, workflows, materialized views, and Moose-managed `ConsumptionApi` endpoints (including the ABI analytics definitions). No general FastAPI routers are mounted here.
...
| Moose entry point | `app/main.py` (Moose + ABI) | `app/main.py` (Moose only) |
| Router location | `app/abi/routers/*` | `bia_backend/routers/*` |
...
- `scripts/abi-api.sh`, `scripts/run-abi-standalone.sh`, `scripts/abi-dev.sh` – Updated to launch the new module.
...
- ABI business logic that must run inside Moose (e.g., `ConsumptionApi` definitions) still resides in `app/abi/`.

## Validation Checklist

- [x] `python -m compileall bia_backend app/main.py`
- [ ] `./scripts/abi-api.sh` → health endpoints respond on port 4300
