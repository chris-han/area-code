...
// Test configuration
const config = {
  enrollmentNumber: "V5702303S0121",
...
const tests = {
  async pageNavigation() {
    ...
    if (window.location.href.includes("azure-billing")) {
      ...
      return {
        status: "FAIL",
        message: "Navigate to Azure Billing page first",
      };
    }
  },
  ...
  async pageElements() {
    ...
    const config =
      utils.findElementByText("Configuration") ||
  ...
  async configurationForm() {
    ...
    if (configSection) {
      ...
      const expander =
        configSection.closest('[data-testid="stExpander"]') ||
    ...
    const enrollmentInput = inputs.find(
      (input) =>
        (input.placeholder &&
          input.placeholder.toLowerCase().includes("enrollment")) ||
        (input.labels &&
    ...
    if (inputs.length > 0 && inputsWorking > 0) {
      ...
      return {
        status: "WARN",
        message: "Configuration form not visible (may be collapsed)",
      };
    } else {
  ...
  async dataDisplay() {
    ...
    const noDataMsg =
      utils.findElementByText("No Azure billing data available") ||
  ...
  async workflowControls() {
    ...
    const testButton = buttons.find(
      (btn) => btn.textContent && btn.textContent.includes("Test Connection")
    );
    ...
    const workflowSection =
      utils.findElementByText("Workflows") ||
    ...
    if (controlsFound >= 2) {
      return {
        status: "PASS",
        message: `${controlsFound} workflow controls found`,
      };
    } else {
      return {
        status: "WARN",
        message: `Only ${controlsFound} workflow controls found`,
      };
    }
  },
  ...
  async interactivity() {
    ...
    if (interactiveElements >= 3) {
      return {
        status: "PASS",
        message: `${interactiveElements} interactive elements`,
      };
    } else {
      return {
        status: "WARN",
        message: `Only ${interactiveElements} interactive elements`,
      };
    }
  },
};
