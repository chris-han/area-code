...
from app.abi_app import get_plugin_manager, get_clickhouse_client, get_plugin_registry
...
from app.abi.plugin_management import (
...
async def test_plugin_configuration_endpoint(
    ...
    try:
        ...
        if normalized_name in {"clickhouse sink", "clickhouse-sink", "clickhouse_sink"}:
            test_results = await _test_clickhouse_connection(config)
        ...
        return PluginTestResponse(
            ...
            test_results={"plugin_name": plugin_name},
        )
    except HTTPException:
        raise
