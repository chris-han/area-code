...
class AzureEAApiClient:
    ...
    def __init__(self, config: AzureApiConfig):
        ...
        logger.info(
            f"Initialized Azure EA API client for enrollment: {self.enrollment_number}"
        )
    ...
    def _make_request(
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                logger.debug(
                    f"Making request to: {url} (attempt {attempt}/{self.max_retries})"
                )
                ...
                logger.warning(
                    f"HTTP error {last_response.status_code} on attempt {attempt}: {str(e)}"
                )
                ...
                if attempt < self.max_retries and self._is_retryable_error(
                    ...
                    logger.error(
                        f"HTTP error {last_response.status_code} for URL: {url}"
                    )
                ...
                logger.warning(
                    f"Connection/timeout error on attempt {attempt}: {str(e)}"
                )
        ...
        if last_exception:
            logger.error(
                f"All {self.max_retries} retry attempts exhausted for URL: {url}"
            )
    ...
    def get_billing_data_url(self, month: str) -> str:
        ...
        """
        # Use the paginated V3 endpoint format that works with the current API
        endpoint = f"{self.enrollment_number}/usage-report/paginatedV3"
        url = self._build_api_url(endpoint)
        return f"{url}?month={month}&pageindex=0&fmt=json"
    ...
    def fetch_billing_data(
        self, month: str, page_url: Optional[str] = None
        ...
        Args:
            ...
            page_url: Optional URL for pagination (custom page URL)
        ...
        try:
            ...
            # Extract billing records based on API response format
            billing_records = []
            next_link = None

            if isinstance(data, dict):
                # Check for paginated V3 API response format
                if "data" in data:
                    billing_records = data["data"]
                    logger.info(
                        f"Retrieved {len(billing_records)} billing records (paginatedV3 format)"
                    )

                    # Handle pagination for V3 API
                    if "nextPageLink" in data and data["nextPageLink"]:
                        next_link = data["nextPageLink"]
                        logger.debug(f"Next page available: {next_link}")
                    elif "hasMoreData" in data and data["hasMoreData"]:
                        # Build next page URL by incrementing page index
                        import re

                        current_page_match = re.search(r"pageindex=(\d+)", url)
                        if current_page_match:
                            current_page = int(current_page_match.group(1))
                            next_page = current_page + 1
                            next_link = re.sub(
                                r"pageindex=\d+", f"pageindex={next_page}", url
                            )
                            logger.debug(f"Next page available: {next_link}")

                # Standard Azure EA API response format (fallback)
                elif "value" in data:
                    billing_records = data["value"]
                    logger.info(
                        f"Retrieved {len(billing_records)} billing records (standard format)"
                    )
                    ...
                    logger.warning("No 'data' or 'value' field found in API response")
                ...
                raise ValueError(
                    f"Unexpected response format from Azure EA API: {type(data)}"
                )
    ...
    def fetch_all_billing_data(self, month: str) -> List[Dict]:
        ...
        try:
            while True:
                ...
                logger.debug(
                    f"Page {page_count}: {len(records)} records, Total: {len(all_records)}"
                )
            ...
            logger.info(
                f"Completed data fetch for {month}: {len(all_records)} total records across {page_count} pages"
            )
            return all_records
            ...
            logger.info(
                f"Partial data retrieved: {len(all_records)} records from {page_count} pages"
            )
            raise
