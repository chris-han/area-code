...
# Individual services
bun run --cwd ufa/apps/web-frontend-foobar dev               # Frontend only
bun run --cwd ufa/services/transactional-supabase-foobar dev # Transactional API only
bun run --cwd ufa/services/analytical-moose-foobar dev       # Analytical API only
bun run --cwd ufa/services/retrieval-elasticsearch-foobar dev # Search API only
```
